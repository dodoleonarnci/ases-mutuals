...
4. **Account hardening** – Layer on Supabase Auth or another SSO provider so users authenticate with their Stanford identity and you can drop the temporary password flow.
5. **Admin dashboards** – Create a protected `src/app/admin` route where organizers can monitor survey completion, trigger re-matching runs, and review email delivery logs.
